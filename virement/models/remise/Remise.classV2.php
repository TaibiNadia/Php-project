<?php 
		...
		function __construct($id,$libelle,$date,$motif,$valid)
		{
			...
			else
			{
				...
				if($this->get_msg_erreur()=='vide')
					...
					throw new Exception('Constructeur class Remise : '.$this->get_msg_erreur());
			}
		}
	//****************************************************** setters ***************************************************
	...
	//*********************** set_motif *************************
		...
		{
			if(!empty($motif)) // est ce qu pwd est renseigné
			{	
				$pwd=trim(htmlspecialchars($motif));
				if(strlen($motif)>30 || strlen($motif)<4) 
				{
					$retour=false;
			...
			{
				$retour=false;
				$this->set_msg_erreur('Le motif est obligatoire  !');					
			}
			return $retour;
		}
